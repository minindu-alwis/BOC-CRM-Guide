...
interface EnterSecureCodeStepProps {
  darkMode: boolean;
}

const EnterSecureCodeStep: React.FC<EnterSecureCodeStepProps> = ({ darkMode }) => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      if (currentIndex < digits.length) {
        ...
        setTimeout(() => {
          setActiveKey(null);
          setCode(prev => prev + digits[currentIndex]);
          currentIndex++;
        }, 300);
      } else {
        if (step === 1) {
      ...
      <div className={`w-full h-full flex flex-col items-center`}>
        <div className={`w-3/4 h-1/2 mx-auto mt-4 ${darkMode ? 'bg-black' : 'bg-blue-900'} rounded-md relative flex flex-col items-center justify-center border-2 ${darkMode ? 'border-gray-600' : 'border-gray-400'}`}>
          <div className={`text-center ${darkMode ? 'text-white' : 'text-white'} p-2 text-sm font-bold mb-2`}>
            {step === 1 ? 'Enter Secure Code / OTP' : 'Re-enter Secure Code'}
          </div>
          
          <div className={`w-3/4 h-10 ${darkMode ? 'bg-gray-800' : 'bg-gray-200'} rounded flex items-center justify-center px-3 mb-4`}>
...
export default EnterSecureCodeStep;