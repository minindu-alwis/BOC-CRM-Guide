...
const EnterIdStep: React.FC<EnterIdStepProps> = ({ darkMode }) => {
  const [id, setId] = useState<string>('');
  const [activeKey, setActiveKey] = useState<number | null>(null);
  
  useEffect(() => {
    const digits = ['9', '7', '2', '8', '3', '0', '4', '1', '5', 'V'];
    let currentIndex = 0;
    
    const interval = setInterval(() => {
      if (currentIndex < digits.length) {
        const keyToPress = isNaN(parseInt(digits[currentIndex])) ? '#' : parseInt(digits[currentIndex]);
        setActiveKey(keyToPress);
        
        setTimeout(() => {
          setActiveKey(null);
          setId(prev => prev + digits[currentIndex]);
          currentIndex++;
        }, 300);
      } else {
      ...
      <div className={`w-full h-full flex flex-col items-center`}>
        <div className={`w-3/4 h-1/2 mx-auto mt-4 ${darkMode ? 'bg-black' : 'bg-blue-900'} rounded-md relative flex flex-col items-center justify-center border-2 ${darkMode ? 'border-gray-600' : 'border-gray-400'}`}>
          ...
          <div className={`w-3/4 h-10 ${darkMode ? 'bg-gray-800' : 'bg-gray-200'} rounded flex items-center justify-start px-3 mb-4`}>
            <span className="text-lg font-mono">{id}</span>
          </div>
          
          <div className="text-xs text-white opacity-70">
            Press # for letters (V, X)
          </div>
        </div>
